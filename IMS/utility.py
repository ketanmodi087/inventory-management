        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    ...
                    title="Stock bajo: artículo %s" % item.ARTICULO,
                    message="Por debajo del punto de reorden en {} (actual: {:,.2f}, ROP: {:,.2f})".format(item.BODEGA, item.CANT_DISPONIBLE,
    ...
    try:
        ...
        for user in users:
            for requirement in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Nueva solicitud: %s" % requirement.SOLICITUD_OC,
                    defaults={
                        "message": "Enviado, aprobación pendiente.",
                        "alert_type": "success",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for requirement in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Solicitud aprobada: %s" % requirement.SOLICITUD_OC,
                    defaults={
                        "message": "Aprobado, orden de compra pendiente.",
                        "alert_type": "info",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for requirement in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Solicitud rechazada: %s" % requirement.SOLICITUD_OC,
                    defaults={
                        "message": "Rechazado, por favor revise.",
                        "alert_type": "error",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for requirement in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Solicitud cancelada: %s" % requirement.SOLICITUD_OC,
                    defaults={
                        "message": "Cancelado, por favor revise.",
                        "alert_type": "warning",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Nueva orden de compra: %s" % item.ORDEN_COMPRA,
                    defaults={
                        "message": "Enviado, aprobación pendiente.",
                        "alert_type": "success",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Orden de compra vencida: %s" % item.ORDEN_COMPRA,
                    defaults={
                        "message": "Orden de compra vencida desde hace {} días.".format(item.overdue_days),
                        "alert_type": "warning",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Alerta de cantidad pedida: %s, %s" % (item['ARTICULO'], item['order_alert']),
                    defaults={
                        "message": "Solicitado: {}, Requerido: {:.0f}".format(item['planned_qty'], item['reorder_qty']),
                        "alert_type": "warning",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Orden de compra aprobada: %s" % item.ORDEN_COMPRA,
                    defaults={
                        "message": "Aprobado, embarque pendiente.",
                        "alert_type": "info",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Orden de compra rechazada: %s" % item.ORDEN_COMPRA,
                    defaults={
                        "message": "Rechazado, por favor revise.",
                        "alert_type": "error",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Orden de compra cancelada: %s" % item.ORDEN_COMPRA,
                    defaults={
                        "message": "Cancelado, por favor revise.",
                        "alert_type": "warning",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Nuevo embarque: %s" % item.EMBARQUE,
                    defaults={
                        "message": "Enviado, aprobación pendiente.",
                        "alert_type": "success",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Embarque aprobado: %s" % item.EMBARQUE,
                    defaults={
                        "message": "Aprobado, en tránsito.",
                        "alert_type": "info",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Embarque cancelado: %s" % item.EMBARQUE,
                    defaults={
                        "message": "Cancelado, por favor revise.",
                        "alert_type": "warning",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Embarque rechazado: %s" % item.EMBARQUE,
                    defaults={
                        "message": "Rechazado, por favor revise.",
                        "alert_type": "error",
                        "timestamp": datetime.now(),
...
def send_softland_sync_alert(module: str):
    ...
    async_to_sync(channel_layer.group_send)(
        ...
        {
            "type": "task_notification",
            "message": "{}: sincronización completada".format(module),
            "timestamp": datetime.now().isoformat(),
            "alert_type": "info",
        }
    )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Nuevo registro de auditoría: %s" % item.AUDIT_TRANS_INV,
                    defaults={
                        "message": "Consecutivo: %s, Aplicación: %s, Referencia: %s" % (item.CONSECUTIVO, item.APLICACION,
                                                                                         item.REFERENCIA),
                        "alert_type": "success",
                        "timestamp": datetime.now(),
                    }
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Nueva transferencia de stock: %s" % item.DOCUMENTO_INV,
                    defaults={
                        "message": "Consecutivo: %s, Aprobado: %s" % (item.CONSECUTIVO,
                                                                      'Sí' if item.APROBADO == 'S' else 'No'),
                        "alert_type": "success",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Transferencia de stock aprobada: %s" % item.DOCUMENTO_INV,
                    defaults={
                        "message": "Aprobado, pendiente de aplicación.",
                        "alert_type": "info",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Transferencia de stock cancelada: %s" % item.DOCUMENTO_INV,
                    defaults={
                        "message": "Cancelado, por favor revise.",
                        "alert_type": "warning",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Transferencia de stock rechazada: %s" % item.DOCUMENTO_INV,
                    defaults={
                        "message": "Rechazado, por favor revise.",
                        "alert_type": "error",
                        "timestamp": datetime.now(),
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title="Transferencia de stock aplicada: %s" % item,
                    defaults={
                        "message": "Transferencia de stock aplicada correctamente!",
                        "alert_type": "info",
                        "timestamp": datetime.now(),
                    }
                )
...
def get_forecast_reviews(existing_stock, forecast_total: float) -> dict:
    ...
    review = {"message": "Stock suficiente disponible", "level": "good",
              "available_percentage": available_perc * 100, "forecast_total": forecast_total,
              "existing_stock": existing_stock}
    if available_perc > 1.5:
        review = {"message": "Exceso de stock", "level": "over stock",
                  "available_percentage": available_perc * 100, "forecast_total": forecast_total,
                  "existing_stock": existing_stock}
    if available_perc < 0.8:
        review = {"message": "El artículo necesita reabastecimiento", "level": "low stock",
                  "available_percentage": available_perc * 100, "forecast_total": forecast_total,
                  "existing_stock": existing_stock}
    if available_perc < 0.4:
        review = {"message": "El artículo necesita reabastecimiento urgente", "level": "critical",
                  "available_percentage": available_perc * 100, "forecast_total": forecast_total,
                  "existing_stock": existing_stock}
    if available_perc < 0:
        review = {"message": "Stock suficiente disponible", "level": "good",
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title=f"Nuevo lote de conteo cíclico: CYCB-{item.id:05d}",
                    defaults={
                        "message": "Programado para: {}".format(item.scheduled_date.strftime("%Y-%m-%d")),
                        "alert_type": "success",
                        "timestamp": datetime.now(),
                    }
                )
    ...
    try:
        ...
        for user in users:
            for item in items:
                Notification.objects.update_or_create(
                    recipient=user, notification_type=notification_type,
                    title=f"Lote de conteo cíclico completado: CYCB-{item.id:05d}",
                    defaults={
                        "message": "Revise el documento INV({}) en el ERP y aplique.".format(item.document_number),
                        "alert_type": "info",
                        "timestamp": datetime.now(),
                    }
                )
